...
# DetectedObjectHTMLFileGenerator.py for YOLOv3
...
class DetectedObjectHTMLFileGenerator:
  ...
  def __init__(self, argv):
    ...
    self.outRoot         = parser.get("OUTPUT",        "foldername")   # C:/Apach24/htdocs/keras-yolo3/
  ...
  def detectAll(self, paired_filenames_list):
    ...
    for imageFilepath in filenames:
      ...
      r_image = self.detector.detect_image(image, fullOutImageFilepath, fullOutCsvFilepath)
    ...
    with open(self.htmlTemplate) as ifs:
      ...
      with open(self.fullOutHtmlFilename, "w") as ofs:
        ...
        while True:
          ...
          if not line:
            break
          if line.find( "<!-- LAST_MODIFIED -->") >= 0:
            now = time.time()
            utc = datetime.utcfromtimestamp(now)
            fmt = utc.strftime("%Y/%m/%d %H:%M:%S %Z %z")
            ofs.write("Last modified (UTC): " + fmt)
...
if __name__=="__main__":
  ...
  try:
    generator = DetectedObjectHTMLFileGenerator(sys.argv)
    generator.run()
